...
namespace ZeroGUI
{
	...
	SDK::UFont* find_font(const std::string& name)
	{
		/* logged fonts:
		ff: TSLFont
		ff : TSLFont_old
		...
		ff : Roboto18
		ff : Roboto
		ff : RobotoDistanceField
		ff : RobotoTiny
		*/
		for (int i = 0; i < SDK::UObject::GObjects->Num(); i++) {
			if (auto item = SDK::UObject::GObjects->GetByIndex(i)) {
				if (item->IsA(SDK::UFont::StaticClass()) && item->GetName() == name)
					return reinterpret_cast<SDK::UFont*>(item);
			}
		}
...
{
    SDK::UFont* font = find_font("Roboto");
    if (!font)
    {
    ...
    // Use s2wc to convert char* to wchar_t*
    wchar_t* wname = s2wc(name);

    // Create FString using the wchar_t constructor
    UC::FString fstring(wname);

    // Don't forget to delete the allocated memory
    delete[] wname;

    canvas->K2_DrawText(font, fstring, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
}
...
void TextCenter(const char* name, FVector2D pos, FLinearColor color, bool outline)
{
    ...
    canvas->K2_DrawText(font, fstring, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
}
